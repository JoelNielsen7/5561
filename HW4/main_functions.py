...
def main_slp_linear():
    ...
    w, b = cnn.train_slp_linear(mini_batch_x, mini_batch_y)
...
def main_slp():
    ...
    im_train, im_test = im_train / 255.0, im_test / 255.0
    mini_batch_x, mini_batch_y = cnn.get_mini_batch(im_train, label_train, batch_size)
    w, b = cnn.train_slp(mini_batch_x, mini_batch_y)
...
def main_mlp():
    ...
    im_train, im_test = im_train / 255.0, im_test / 255.0
    mini_batch_x, mini_batch_y = cnn.get_mini_batch(im_train, label_train, batch_size)
    w1, b1, w2, b2 = cnn.train_mlp(mini_batch_x, mini_batch_y)
    ...
    for i in range(num_test):
        x = im_test[:, [i]]
        pred1 = cnn.fc(x, w1, b1)
        pred2 = cnn.relu(pred1)
        y = cnn.fc(pred2, w2, b2)
        l_pred = np.argmax(y)
        confusion[l_pred, label_test[0, i]] = confusion[l_pred, label_test[0, i]] + 1
